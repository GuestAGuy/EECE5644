...
class GaussianClassifier:
    ...
    def eval_gaussian_log_pdf(self, x, mu, Sigma):
        """numerically stable implementation of eval_gaussian_pdf"""
        n = x.shape[0]
        sign, log_det = np.linalg.slogdet(Sigma)
        log_C = -0.5 * (n * np.log(2 * np.pi) + log_det)
        a = x - mu.reshape(-1, 1)
        b = pinv(Sigma) @ a
        log_px = log_C - 0.5 * np.sum(a * b, axis=0)
        return log_px
    ...
    def predict(self, X):
        ...
        for i, class_label in enumerate(self.classes):
            ...
            likelihood = self.eval_gaussian_log_pdf(X_t, mu, Sigma)
